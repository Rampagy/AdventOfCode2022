...
# breadth first search to get all positions within disctance of start
def GetPositionsWithinDistance(start, distance, diagonals=False):
    # returns list of Position objects
    # start should be position objects
    # distance is interpreted based on the diagonals flag (only manhattan is currently allowed)
    # diagonals not currently allowed
    already_visited = [] #{(start.y, start.x)}
    contiguous_positions = []
    nogo = set()
    ...
    while (len(already_visited) > 0):
        [_, [row, col]] = heapq.heappop(already_visited) #row, col = already_visited.pop()
        nogo.add((row, col))
        if heuristic(Position(col, row), start) <= distance:
            contiguous_positions.append(Position(col, row))
        else:
            # started looking further than distance away: EXIT
            break
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                if (new_row, new_col) not in nogo:
                    dist_from_start = heuristic(Position(new_col, new_row), start)
                    heapq.heappush(already_visited, (dist_from_start, (new_row, new_col))) # already_visited.add((new_row, new_col))
                #elif map[new_row][new_col] in invalid:
                #    nogo.add((new_row, new_col))
