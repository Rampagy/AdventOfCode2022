...
if __name__=='__main__':
    ...
    sand_bags = set()
    sand_bag_valid = True
    while sand_bag_valid:
        sand_bag = sand_start.copy()

        while sand_bag.x >= min_map_bounds.x or \
                sand_bag.x <= max_map_bounds.x or \
                sand_bag.y >= min_map_bounds.y or \
                sand_bag.y <= max_map_bounds.y:
            ...
            if sand_bag.x < min_map_bounds.x or \
                    sand_bag.x > max_map_bounds.x or \
                    sand_bag.y < min_map_bounds.y or \
                    sand_bag.y > max_map_bounds.y:
                sand_bag_valid = False
                break
            elif down_pos not in rock_map and down_pos not in sand_bags:
                sand_bag = down_pos
            elif diag_left_pos not in rock_map and diag_left_pos not in sand_bags:
                sand_bag = diag_left_pos
            elif diag_right_pos not in rock_map and diag_right_pos not in sand_bags:
