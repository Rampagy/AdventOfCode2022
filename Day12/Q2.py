...
if __name__ == '__main__':
    ...
    shortest_path_dist = 9999
    shortest_path = []
    for start in starts:
        try:
            path = djikstra_search(search_map, start, END)
        except KeyError:
            # error is thrown if the start position is on an island that it can't escape
            # therefore skip it and move on to the next starting position
            continue
